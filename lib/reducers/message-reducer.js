...
function reduceMessageStore(
  ...
  if (action.type === logInActionTypes.success) {
    ...
    for (const threadID in threads) {
      for (const msgID of threads[threadID].messageIDs) {
        displayedMessages.add(msgID);
      }
    }
    const threadIDsNeedResorting = new Set();
    ...
    for (const id in actionPayloadMessages) {
      ...
      } else if (!displayedMessages.has(id)) {
        newThreads = {
          ...newThreads,
          [threadID]: {
            ...newThreads[threadID],
            messageIDs: [...newThreads[threadID].messageIDs, id],
          },
        };
        threadIDsNeedResorting.add(threadID);
      }
    }

    for (const threadID of threadIDsNeedResorting) {
      newThreads[threadID].messageIDs = sortMessageIDs(actionPayloadMessages)(
        newThreads[threadID].messageIDs,
      );
    }
