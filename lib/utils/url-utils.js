...
const threadRegex = new RegExp('(/|^)thread/(.+)(/|$)', 'i');
...
const threadCreationRegex = new RegExp('(/|^)thread/new/(.*)(/|$)', 'i');
...
function infoFromURL(url: string): URLInfo {
  ...
  const createThreadMatches = threadCreationRegex.exec(url);
  ...
  if (threadMatches) {
    if (createThreadMatches) {
      returnObj.threadCreation = true;
      const users = createThreadMatches[2].replace(/\/$/, '');
      returnObj.selectedUserList = users?.split(',') ?? [];
    } else {
      returnObj.thread = threadMatches[2].replace(/\/$/, '');
    }
  }
  if (verifyMatches) {
