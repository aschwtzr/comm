...
SCP_SOURCE_SERVER=$(echo $SCP_SOURCE | cut -d: -f1)
SCP_SOURCE_FOLDER=$(echo $SCP_SOURCE | cut -d: -f2)
...
while true; do
  TOTAL_USAGE=$(sudo du -cs "$SCP_DEST_FOLDER"/comm.*.sql.gz | grep total | awk '{ print $1 }')
  if [[ $TOTAL_USAGE -le $MAX_DISK_USAGE_KB ]]; then
    break
  fi
  BACKUP_COUNT=$(ls -hla "$SCP_DEST_FOLDER"/comm.*.sql.gz | wc -l)
