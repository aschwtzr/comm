...
void DatabaseManagerBase::innerBatchWriteItem(
  ...
  std::vector<Aws::DynamoDB::Model::WriteRequest>::iterator chunkPositionStart,
      chunkPositionEnd;
  for (size_t i = 0; i < writeRequests.size(); i += MAX_DYNAMODB_BATCH_ITEMS) {
    chunkPositionStart = writeRequests.begin() + i;
    chunkPositionEnd = writeRequests.begin() +
        std::min(writeRequests.size(), i + MAX_DYNAMODB_BATCH_ITEMS);
