...
TEST_F(
    DatabaseManagerTest,
    UpdatedAndFoundDeviceSessionItemCheckpointTimeIsSame) {
  const database::DeviceSessionItem item(
      "bc0c1aa2-bf09-11ec-9d64-0242ac120002",
      "mobile:EMQNoQ7b2ueEmQ4QsevRWlXxFCNt055y20T1PHdoYAQRt0S6TLzZWNM6XSvdWqxm",
      "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC9Q9wodsQdZNynbTnC35hA4mFW"
  ...
  EXPECT_EQ(
      ...
      true);
  database::DatabaseManager::getInstance().putSessionItem(item);
  const size_t newCheckpointTime = 123456789;
  database::DatabaseManager::getInstance().updateSessionItemCheckpointTime(
      item.getSessionID(), newCheckpointTime);
  std::shared_ptr<database::DeviceSessionItem> foundUpdatedItem =
      database::DatabaseManager::getInstance().findSessionItem(
          item.getSessionID());
  EXPECT_NE(foundUpdatedItem, nullptr);
  EXPECT_EQ(foundUpdatedItem->getCheckpointTime(), newCheckpointTime);
  database::DatabaseManager::getInstance().removeSessionItem(
      item.getSessionID());
}
